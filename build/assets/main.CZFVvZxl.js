...
function openFilterPopup(header, type, field, render, closeFilterPopup) {
	...
	popup.innerHTML = `
    ${`<input class="filter-input" type="text" value="${state.filters[type][field] || ""}" placeholder="Filter..." />`}
...
function createTableModule(deps) {
	const { applyFilters, applySort, getCurrentPeriodData, getEmployeeMetrics, getProjectMetrics, getTotalEstimatedIncome, saveData, render, openDetailsPopup } = deps;
...
//# sourceMappingURL=main.CZFVvZxl.js.map